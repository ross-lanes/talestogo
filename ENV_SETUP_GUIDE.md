...
TALES uses LLM APIs to collect and analyze AI responses. API keys are configured **exclusively through environment variables** (not via the Admin UI). You need at least one of these:
...
| `ENCRYPTION_KEY` | Encrypts sensitive data | ✅ Yes (already set) |
...
| `GEMINI_API_KEY` | Gemini data collection + analysis + web search | ⚠️ One LLM key required |
| `OPENAI_API_KEY` | ChatGPT data collection | ⚠️ One LLM key required |
| `ANTHROPIC_API_KEY` | Claude data collection | ⚠️ One LLM key required |
| `PERPLEXITY_API_KEY` | Perplexity data collection + web search | ❌ Optional |
| `DATABASE_URL` | PostgreSQL connection | ❌ Optional (SQLite used by default) |
| `REDIS_URL` | Redis for background tasks | ❌ Optional |

**Note:** Only providers with API keys configured in the environment will appear in the application. You can add custom LLM providers by defining additional environment variables (e.g., `MISTRAL_API_KEY`) and configuring them in the Admin UI.
...
**Cause:** No LLM API keys configured in environment variables

**Solution:**
1. Get at least one LLM API key (recommend Gemini)
2. Add it to backend `.env` (e.g., `GEMINI_API_KEY=your-key-here`)
3. Restart backend server
4. Check Admin > LLM Configuration to verify the provider appears

**Note:** API keys can ONLY be configured via environment variables. Only providers with valid keys will appear in the Admin UI.
