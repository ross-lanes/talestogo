...
/**
 * Positioning Analytics Tests
 *
 * These tests verify the Positioning analytics page functionality.
...
test.describe('Positioning Page', () => {

  test.beforeEach(async ({ page }) => {
    // Navigate to positioning page
    await page.goto('/analytics/positioning');
  });

  test('should display page title', async ({ page }) => {
    // Verify page title
    const heading = page.locator('h1, h2').first();
    await expect(heading).toContainText(/Positioning Analysis/i);
  });
  ...
  test('should display positioning categories breakdown', async ({ page }) => {
  ...
  test('should display positioning categories', async ({ page }) => {
    // Wait for data to load
    await page.waitForLoadState('networkidle');

    // Check for common positioning categories (if data exists)
    const categories = ['Leader', 'Challenger', 'Niche', 'Innovator', 'Not Mentioned'];
    ...
    // At least one category should be present if there's data
    const chartExists = await page.locator('svg.recharts-surface').count();
    if (chartExists > 0) {
  ...
  test('should handle no data state gracefully', async ({ page }) => {
    ...
    const noDataMessage = page.locator('text=/No positioning data available/i');
    const chartExists = await page.locator('svg.recharts-surface').count();
...
test.describe('Positioning Interactivity', () => {
